...
Config is env-driven via `packages/config` (zod). Dev defaults ship in `docker-compose.yml` (`--profile dev`). A full reference template lives at `Real Estate Web RD/env.example`. Key vars:
...
`Real Estate Web RD/` holds the source research/design layer (blueprint, not a mirror of the implemented repo): PRD, TRD, architecture, AI/RAG RD, Twenty CRM RD, CMS content pipeline, scheduler/email/social, analytics, data model/API, devops runbook, security/compliance, QA/UAT, roadmap, risk register, decisions lock, an OpenAPI 3.1.0 contract (`api_contracts.openapi.yaml`), a skeleton compose (`docker-compose.skeleton.yml` differs from `docker-compose.yml`), `env.example`, and `schema.sql` (includes future tables not yet in Drizzle). `pnpm check:openapi` validates Phase 1A + platform paths implemented today; `pnpm check:compose` validates skeleton + `docker-compose.yml` profiles.
