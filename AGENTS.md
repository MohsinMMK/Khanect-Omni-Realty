...
Other:
- `plans/`: implementation plans 001-004 (status `TODO`), generated by the `improve` skill. See `plans/README.md`.
- `Real Estate Web RD/`: research/design docs, OpenAPI 3.1.0 contract, skeleton compose, `env.example`, `schema.sql`.
...
`plans/` holds self-contained plans for other agents to execute. Read a plan fully before starting, honor its STOP conditions, run every verification command, and update the status row in `plans/README.md` when done. Current plans: 001 (web verification baseline), 002 (split admin app into modules), 003 (productionize content approval desk), 004 (reconcile Select/Drawer with shadcn Base UI). Execution order: 001 → 002 → 003; 004 after 001 independently.
